...
// StartupNotifier manages startup notifications to the GraphLinq network registry
...
func NewStartupNotifier(endpoint string, logger interface{ Info(string, ...interface{}) }, rpcPort uint64) *StartupNotifier {
...
func (sn *StartupNotifier) NotifyStartup(moniker string) error {
	...
	sn.logger.Info("Successfully notified network registry of blockchain startup", "moniker", moniker, "endpoint", sn.endpoint, "rpc_port", sn.rpcPort)
	return nil
}
...
func (sn *StartupNotifier) getRPCURL() string {
	return "http://localhost:" + strconv.FormatUint(sn.rpcPort, 10)
}
